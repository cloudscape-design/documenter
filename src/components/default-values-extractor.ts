...
export default function extractDefaultValues(component: DeclarationReflection): Record<string, any> {
  ...
  if (callSignature) {
    return extractFromSource(fs.readFileSync(component.sources![0].fullFileName, 'utf-8'));
  }
...
export function extractFromSource(sourceCode: string, matchingName): Record<string, any> {
